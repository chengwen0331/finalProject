...
if (isset($_POST['buyerid']) && isset($_POST['orderbill'])){
	...
	$sqlorderdetails = "SELECT * FROM `apps_orderdetails` INNER JOIN `apps_items` ON apps_orderdetails.item_id = apps_items.item_id WHERE apps_orderdetails.buyer_id = '$buyerid' AND apps_orderdetails.order_bill = '$orderbill' AND apps_orderdetails.seller_id = '$sellerid'";
}else{
    $sqlorderdetails = "NA";
}
