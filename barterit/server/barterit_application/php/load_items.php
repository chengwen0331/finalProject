...
if(isset($_POST['userid'])){
    $userid = $_POST['userid'];
    $sqlloaditems = "SELECT * FROM `apps_items` WHERE user_id = '$userid'";
    $sqlcart = "SELECT * FROM `apps_carts` WHERE user_id = '$userid'";
}
else if(isset($_POST['search'])){
    $search = $_POST['search'];
    $sqlloaditems = "SELECT * FROM `apps_items` WHERE item_name LIKE '%$search%'";
    $sqlcart = "SELECT * FROM `apps_carts` WHERE user_id = '$cartuserid'";
}
else if(isset($_POST['type'])){
    $type = $_POST['type'];
    $sqlloaditems = "SELECT * FROM `apps_items` WHERE item_type = '$type'";
    $sqlcart = "SELECT * FROM `apps_carts` WHERE user_id = '$cartuserid'";
}
else{
    $sqlloaditems = "SELECT * FROM `apps_items`";
    $sqlcart = "SELECT * FROM `apps_carts` WHERE user_id = '$cartuserid'";
}
...
if ($result->num_rows > 0) {
    $items["items"] = array(); //change to items
...
while ($row = $result->fetch_assoc()) {
        ...
        $itemlist['item_con'] = $row['item_con'];
        $itemlist['item_trade'] = $row['item_trade'];
    ...
    $response = array('status' => 'success', 'data' => $items, 'numofpage'=>"$number_of_page",'numberofresult'=>"$number_of_result", 'sql'=>$sqlloaditems, 'cartqty'=> $totalcart);
    sendJsonResponse($response);
}else{
     $response = array('status' => 'failed', 'data' => null);
    sendJsonResponse($response);
